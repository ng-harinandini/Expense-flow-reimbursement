"""Rule-extraction prompt v3 — closes three gaps a real Finance review of v2's output found.

Builds on ``v2`` (kept unmodified for rollback via ``AI_RULE_EXTRACTION_PROMPT_VERSION=v2``).
A Finance reviewer's edits to a real v2 extraction batch surfaced three concrete, general
failure patterns — none tied to this one document's wording:

1. **Anchoring on illustrative literals beyond the worked example.** The reviewer had to remove
   an invented ``escalate_above: 1000`` / ``route_to: "Finance Shared Services"`` / notify list
   the policy never stated, and a ``settlement_days: 30`` that leaked onto a meals rule from the
   worked example's own action values. Those are exactly the placeholder literals sitting in the
   ``--- ACTIONS ---`` / ``--- CONDITIONS ---`` reference tables and the worked example's action
   fields — v2 only disclaimed the worked example's *expense_limit* numbers, not these.
2. **The middle of a multi-row table gets dropped.** A 3-tier grade table (first tier, middle
   tier, last tier) came back missing only the middle row — a classic primacy/recency gap.
3. **Prose-only rules lose out to table rows.** A trailing sentence after a table (a secondary
   cap) and two rules written as plain paragraphs in procedural sections were all missed, while
   every table-shaped rule survived.
"""
...
2. BE EXHAUSTIVE, IN TABLES AND IN PROSE ALIKE. Extract EVERY rule, limit, threshold, eligibility
   condition, approval requirement, exception, exclusion and caveat stated anywhere in this
   window — including ones that appear well past the midpoint of the text. A procedural or
   approval rule written as an ordinary sentence in an administrative section (workflow,
   approvals, documentation, fraud screening, appeals) is exactly as extractable as a numeric
   limit in a table row, and must not be skipped just because it has no dollar amount attached.
   Missing a clause is the worst possible error, regardless of where it appears or what it looks
   like.
...
4. ONE OBLIGATION PER RULE, AND KEEP DISTINCT RULES SEPARATE. If a paragraph, section, or table
   ...
   heading just because you already emitted one for it. When a table or list has more than two
   rows or entries under one heading, treat EVERY row as its own candidate, including the ones in
   the middle — it is a common and serious error to extract only the first and last row of a
   table (e.g. the first grade tier and the most senior one) and silently skip the ones between.
   A trailing sentence AFTER a table, stating a related but separate cap or condition, is its own
   rule too, not something to fold into the table's rule or skip.
...
7. NOTHING IN THESE INSTRUCTIONS IS THIS DOCUMENT'S DATA — DO NOT ANCHOR ON ANY OF IT. Every
   number, role name, routing destination and rate shown below — in this rule list, in the
   CONDITIONS and ACTIONS reference tables, and in the worked example — exists ONLY to show you
   the shape of a field. None of it is real. If this window does not itself state an escalation
   threshold, an approver name, a routing destination, a settlement period, or a mileage/per-diem
   rate, the corresponding field is null and stays null — even though the reference tables and
   the worked example show values like "1000", "Finance Shared Services", "Reporting Manager", or
   "settlement_days: 30" as illustrations of what the field would look like if stated. Copying one
   of those illustrative values into a real rule is exactly as serious an error as inventing a
   number from nothing. The same applies when a limit is expressed as a variable, a formula, a
   published/external rate, or a reference to another table rather than a literal number stated
   on this page (e.g. "current published mileage rate", "150% of the standard government
   per-diem", "per the union-negotiated rate card"): set "expense_limit" to null and put the exact
   reference phrase, verbatim, in "limit_expression" — never substitute a number from elsewhere.
...
--- CONDITIONS (illustrative shapes only — never real values, see rule 7) ---
...
--- ACTIONS (illustrative shapes only — never real values, see rule 7) ---
...
priority, and flags the two genuine ambiguities instead of guessing. Note also that the first
rule's "settlement_days": 30 comes directly from THIS example's own "within 30 days" sentence —
it is illustrative of the field, not a real-world default; a real meals rule with no stated
settlement period gets no "settlement_days" key at all. This is one illustrative paragraph, not
the full window — a real window is longer and will contain many more such rules across many more
sections, including ones in plain prose and ones in the middle of a longer table; extract all of
them, following rules 1, 2 and 4 above.
